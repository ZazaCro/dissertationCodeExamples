...
#include "turbulentTemperatureTwoPhaseRadCoupledMixedBiotFvPatchScalarField.H"
...
<
    Foam::compressible::
    turbulentTemperatureTwoPhaseRadCoupledMixedBiotFvPatchScalarField::regionType
>
Foam::compressible::
turbulentTemperatureTwoPhaseRadCoupledMixedBiotFvPatchScalarField::regionTypeNames_
{
...
<
    Foam::compressible::
    turbulentTemperatureTwoPhaseRadCoupledMixedBiotFvPatchScalarField::KMethodType
>
Foam::compressible::
turbulentTemperatureTwoPhaseRadCoupledMixedBiotFvPatchScalarField::KMethodTypeNames_
{
...
turbulentTemperatureTwoPhaseRadCoupledMixedBiotFvPatchScalarField::
kappa
(
...
turbulentTemperatureTwoPhaseRadCoupledMixedBiotFvPatchScalarField
(
    const turbulentTemperatureTwoPhaseRadCoupledMixedBiotFvPatchScalarField& psf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
void turbulentTemperatureTwoPhaseRadCoupledMixedBiotFvPatchScalarField::
updateCoeffs()
{
    if (updated())
    {
    ...
    const turbulentTemperatureTwoPhaseRadCoupledMixedBiotFvPatchScalarField&
        nbrField = refCast
            <const turbulentTemperatureTwoPhaseRadCoupledMixedBiotFvPatchScalarField>
            (
    ...
    if (regionType_ == solid)
    {
        ...
        // Information to debug added by Robin 04/12/2020
        scalarField alphaLkNbr;
        alphaLkNbr = alphal*liquid.kappaEff(samplePatchi);
        mpp.distribute(alphaLkNbr);
        
        scalarField alphaVkNbr;
        alphaVkNbr = alphav*vapor.kappaEff(samplePatchi);
        mpp.distribute(alphaVkNbr);

        scalarField deltaF;
        deltaF = nbrPatch.deltaCoeffs();
        mpp.distribute(deltaF);
        
        BiV_ = KDeltaVapNbr/KDelta;
        BiL_ = KDeltaLiqNbr/KDelta;
        
        dfByds_ = alphaVkNbr/kappa(Tp)*((this->patchInternalField() - TvNbr)/
               (this->patchInternalField() - Tp) - 1)
            + alphaLkNbr/kappa(Tp)*((this->patchInternalField() - TcNbr)/
               (this->patchInternalField() - Tp) - 1);
        
        // deltaCoeffs gives inversed value
        erNu_ = dfByds_ - patch().deltaCoeffs()/deltaF;
    ...
    else if (regionType_ == fluid)
    {
        ...
        scalarField ks;
        ks = nbrField.kappa(nbrField);
        mpp.distribute(ks);
        
        scalarField deltaS;
        deltaS = nbrPatch.deltaCoeffs();
        mpp.distribute(deltaS);
        
        BiV_ = KdeltaVap/KDeltaNbr;
        BiL_ = KdeltaLiq/KDeltaNbr;
        
        dfByds_ = alphav*vapor.kappaEff(patchi)/ks*((TcNbr - Tv.patchInternalField())/
               (TcNbr - Tp) - 1)
            + alphal*liquid.kappaEff(patchi)/ks*((TcNbr - this->patchInternalField())/
               (TcNbr - Tp) - 1);
      
        // deltaCoeffs gives inversed value
        erNu_ = dfByds_ - deltaS/patch().deltaCoeffs();
...
void turbulentTemperatureTwoPhaseRadCoupledMixedBiotFvPatchScalarField::write
(
    Ostream& os
) const
{
    ...
    BiV_.writeEntry("BiV", os);
    BiL_.writeEntry("BiL", os);
    dfByds_.writeEntry("dfByds", os);
    erNu_.writeEntry("erNu", os);
}
